            ...
            for (int l1 = j; l1 <= i1; l1++)
            {
                ...
                for (int i2 = k; i2 <= j1; i2++)
                {
                    ...
                    for (int j2 = l; j2 <= k1; j2++)
                    {
                        ...
                        {
                        	if (genunder != 0) {
                        		if (par1World.getBlock(l1, i2, j2) == Blocks.lava){ //TODO:LavaStill
                        			par1World.setBlock(l1, i2 - 1, j2, Block.getBlockById(minableBlockId), metadata, 2);
                        		}
                        	}else{
                        		Random rand = new Random();

                        		if ((rand.nextInt(99)+1) < 50){
                        			par1World.setBlock(l1, i2, j2, Block.getBlockById(minableBlockId), metadata, 2);
                        		}
                        	}
                        }
                    }
